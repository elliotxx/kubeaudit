...
// AuditSingleManifest audits the Kubernetes resources in the provided manifest
func (a *Kubeaudit) AuditSingleManifest(manifest io.Reader) (*Report, error) {
	manifestBytes, err := io.ReadAll(manifest)
	if err != nil {
		return nil, err
	}

	resource, err := getResourcesFromSingleManifest(manifestBytes)
	if err != nil {
		return nil, fmt.Errorf("failed to get resources from manifest: %w", err)
	}

	results, err := auditResources([]KubeResource{resource}, a.auditors)
	if err != nil {
		return nil, err
	}
